...
void cambiar_feromonas(){
    ...
    vector<pair<int,vector<int> > > ranked;
    vector<int>tem;
    int posicion=0;
    ...
    int ranked_=1;
    int pos=0;
    vector<int> fitnes_visitado;
    int fitnes_sig=fitness(caminos_rrecorridos_hormigas[1]);

    for(int x=0;x<caminos_rrecorridos_hormigas.size();x++){
        ranked.push_back(make_pair(1,caminos_rrecorridos_hormigas[x]));
        for(int y=0;y<caminos_rrecorridos_hormigas.size();y++){
            if(fitness(ranked[x].second)>fitness(caminos_rrecorridos_hormigas[y])){
                ranked[x].first++;
            }
        }
    }
    cout<<"RANKED \n";
     for(int x=0;x<ranked.size();x++){
        cout<<"hormiga  "<<x+1<<" = [";
        for(int y=0;y<ranked[0].second.size();y++){
            char m=ranked[x].second[y]+65;
            cout<<m<<" ";
        }cout<<"] ranked= "<<ranked[x].first<<endl;
    }cout<<endl;
            ...
            if(x!=y){
                ...
                for(int z=0;z<cantidad_hormigas;z++){
                    ...
                    if(buscar_camino_hormiga(z,x,y)==true){
                        //cout<<"\nHORMIGA "<<z+1<<endl;
                        suma += (Q/fitness(caminos_rrecorridos_hormigas[z]))*(w-abs(ranked[z].first));
                        //char m=(char)x+65,n=(char)y+65;
                        //cout<<"["<<m<<" "<<n<<"] "<<" suma "<<Q/fitness(caminos_rrecorridos_hormigas[z])
                        //<<"*"<<w-abs(ranked[z].first)<<" = "<<suma<<endl;
                    }
                }
                for(int z=0;z<1;z++){
